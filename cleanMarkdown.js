...
function cleanup (sourceFile, destFile) {
  ...
  var metadataFile = sourceFile.replace('index.md', 'metadata.json')
  ...
  let imageRegEx = /!\[.+?\]\(img\/(.+?)\)\n*\[IMAGEINFO\]:\s*(.+?)\.*,\s*(.+?)\n/g
  let imageMatches, oldImgName, imgSrc
  ...
  markdown = markdown.replace(imageRegEx, `<img src="/images/${metadata.id}/$2" alt="$3">\n`)
  ...
  markdown = markdown.replace(/(\[ICON HERE\])(.*?)!\[(.*?)]\((.*?)\)/g, '<img src="$4" style="width:20pxheight:20px" alt="$3"> $2')
